...
public class EtcBlock19 : BlockAbility
{
    [SerializeField] private float durationMultiplier = 2f; // Duration 배율 (인스펙터에서 설정, 기본값 0.5)
    ...
    void Start()
    {
        ...
        descriptionTemplate = "Duration is set to {0:P0} of base durability, and deals {1} + player attack damage (or {2} + player attack damage on critical hit) to the nearest enemy.";
    ...
    {
        Description = string.Format("Duration이 기본 내구도의 {0:P0}로 설정되고, 가장 가까운 적에게 {1} + 플레이어 공격력 데미지(치명타 시 {2} + 플레이어 공격력)를 주며, {3} 방어력을 얻음.", durationMultiplier, AttackDamage, CriticalHitDamage, Defense); // 설명 업데이트
    }
